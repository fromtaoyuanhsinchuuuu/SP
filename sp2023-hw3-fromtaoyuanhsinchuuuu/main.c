 ...
 */
void init_threads(int fib_arg, int fact_arg, int bank1_arg, int bank2_arg, int fact2_arg, int bank3_arg) {
    // char padding[64];
    if (fib_arg > 0)
	...
	if (fact2_arg > 0)
		thread_create(factorial, 4, fact2_arg);
	if (bank3_arg)
		thread_create(bank_operation, 5, bank3_arg);		
}
...
int main(int argc, char *argv[]) {
	...
	int fact2_arg = atoi(argv[6]);
	int bank3_arg = atoi(argv[7]);
    unbuffered_io();
    init_signal();
    init_bank();
    init_threads(fib_arg, fact_arg, bank1_arg, bank2_arg, fact2_arg, bank3_arg);
    start_threading();
	return 0;
}
